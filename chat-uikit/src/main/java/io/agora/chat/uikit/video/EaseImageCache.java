...
package io.agora.chat.uikit.video;
 ...
 * {@link EaseImageWorker} class and its subclasses. Use
 * {@link EaseImageCache#getInstance(FragmentManager, ImageCacheParams)}
 * to get an instance of this class, although usually a cache should be added
 * directly to an {@link EaseImageWorker} by calling
 * {@link EaseImageWorker#addImageCache(FragmentManager, ImageCacheParams)}
 * .
 */
public class EaseImageCache {
    private static final String TAG = EaseImageCache.class.getSimpleName();
     ...
     */
    private EaseImageCache(ImageCacheParams cacheParams) {
        init(cacheParams);
    }

    /**
     * Return an {@link EaseImageCache} instance. A {@link RetainFragment} is used
     ...
     * not exist
     */
    public static EaseImageCache getInstance(FragmentManager fragmentManager,
        ...
        EaseImageCache imageCache = (EaseImageCache) mRetainFragment.getObject();

        // No existing ImageCache, create one and store it in RetainFragment
        if (imageCache == null) {
            imageCache = new EaseImageCache(cacheParams);
        ...
        if (mCacheParams.memoryCacheEnabled) {
            ...
            mMemoryCache = new LruCache<String, BitmapDrawable>(
                ...
                protected void entryRemoved(boolean evicted, String key,
                    ...
                    if (EaseRecyclingBitmapDrawable.class.isInstance(oldValue)) {
                        // The removed entry is a recycling drawable, so notify
                        // it
                        // that it has been removed from the memory cache
                        ((EaseRecyclingBitmapDrawable) oldValue).setIsCached(false);
                    } else {
    ...
    public void addBitmapToCache(String data, BitmapDrawable value) {
        ...
        if (mMemoryCache != null) {
            if (EaseRecyclingBitmapDrawable.class.isInstance(value)) {
                // The removed entry is a recycling drawable, so notify it
                // that it has been added into the memory cache
                ((EaseRecyclingBitmapDrawable) value).setIsCached(true);
            }
